...
//  FLMenuView
//
//  Created by 乐升平 on 17/10/26. https://github.com/SPStore/FLMenuView
...
typedef NS_ENUM(NSInteger, FLMenuViewTrackerStyle) {
    FLMenuViewTrackerStyleLine = 0,                  // 下划线,默认与item等宽
    FLMenuViewTrackerStyleLineLongerThanItem,        // 下划线,比item要长(长度为item的宽+间距)
    FLMenuViewTrackerStyleLineAttachment,            // 下划线“依恋”样式，此样式下默认宽度为字体的pointSize，你可以通过trackerWidth自定义宽度
    FLMenuViewTrackerStyleRoundedRect,               // 圆角矩形
    FLMenuViewTrackerStyleRect,                      // 矩形
    FLMenuViewTrackerStyleTextZoom NS_ENUM_DEPRECATED_IOS(6_0, 6_0, "该枚举值已经被废弃，请用“selectedItemZoomScale”属性代替"), // 缩放(该枚举已经被废弃,用属性代替的目的是让其余样式可与缩放样式配套使用。如果你同时设置了该枚举和selectedItemZoomScale属性，selectedItemZoomScale优先级高于FLMenuViewTrackerStyleTextZoom
    FLMenuViewTrackerStyleNothing                    // 什么样式都没有
};

typedef NS_ENUM(NSInteger, FLMenuViewPermutationWay) {
    FLMenuViewPermutationWayScrollAdaptContent = 0,   // 自适应内容,可以左右滑动
    FLMenuViewPermutationWayNotScrollEqualWidths,     // 等宽排列,不可以滑动,整个内容被控制在pageMenu的范围之内,等宽是根据pageMenu的总宽度对每个item均分
    FLMenuViewPermutationWayNotScrollAdaptContent     // 自适应内容,不可以滑动,整个内容被控制在pageMenu的范围之内,这种排列方式下,自动计算item之间的间距,itemPadding属性无效
};

typedef NS_ENUM(NSInteger, FLMenuViewTrackerFollowingMode) {
    FLMenuViewTrackerFollowingModeAlways = 0,   // 外界scrollView拖动时，跟踪器时刻跟随外界scrollView移动
    FLMenuViewTrackerFollowingModeEnd,     // 外界scrollVie拖动w结束后，跟踪器才开始移动
    FLMenuViewTrackerFollowingModeHalf     // 外界scrollView拖动距离超过屏幕一半时，跟踪器开始移动
};
...
- (void)pageMenu:(FLMenuView *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex;

// 右侧的功能按钮被点击的代理方法
- (void)pageMenu:(FLMenuView *)pageMenu functionButtonClicked:(UIButton *)functionButton;
@end
...
// item之间的间距，默认30；当排列方式permutationWay为‘FLMenuViewPermutationWayNotScrollAdaptContent’时此属性无效，无效是合理的，不可能做到“不可滑动且自适应内容”然后间距又自定义，这2者相互制约；
...
@property (nonatomic, assign) FLMenuViewPermutationWay permutationWay; // 排列方式
...
// 跟踪器的跟踪模式
@property (nonatomic, assign) FLMenuViewTrackerFollowingMode trackerFollowingMode;
...
// 选中的item缩放系数，默认为1，为1代表不缩放，[0,1)之间缩小，(1,+∞)之间放大，(-1,0)之间"倒立"缩小，(-∞,-1)之间"倒立"放大，为-1"倒立不缩放",如果依然使用了废弃的FLMenuViewTrackerStyleTextZoom样式，则缩放系数默认为1.3
@property (nonatomic) CGFloat selectedItemZoomScale;
@property (nonatomic, assign) BOOL needTextColorGradients; // 是否需要文字渐变,默认为YES

@property (nonatomic, weak) id<FLMenuViewDelegate> delegate;
...
/* 1.让跟踪器时刻跟随外界scrollView滑动,实现了让跟踪器的宽度逐渐适应item宽度的功能;
   2.这个方法用于外界的scrollViewDidScroll代理方法中，如
    ...
    3.如果外界设置了FLMenuView的属性"bridgeScrollView"，那么外界就可以不用在scrollViewDidScroll方法中调用这个方法来实现跟踪器时刻跟随外界scrollView的效果,内部会自动处理; 外界对FLMenuView的属性"bridgeScrollView"赋值是实现此效果的最简便的操作
...
// 默认NO;关闭跟踪器的跟随效果,在外界传了scrollView进来或者调用了moveTrackerFollowScrollView的情况下,如果为YES，则当外界滑动scrollView时，跟踪器不会时刻跟随,只有滑动结束才会跟随;  3.4.0版本开始被废弃，但是依然能使用,使用后相当于设置了FLMenuViewTrackerFollowingModeEnd枚举值
