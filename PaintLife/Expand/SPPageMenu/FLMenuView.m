...
@interface FLMenuViewScrollView : UIScrollView
@end

@implementation FLMenuViewScrollView
...
@interface FLMenuViewLine : UIImageView
@property (nonatomic, copy) void(^hideBlock)(void);

@end

@implementation FLMenuViewLine
...
@interface FLMenuViewItem : UIButton
...
@property (nonatomic, assign) FLMenuViewTrackerStyle trackerStyle;
...
@property (nonatomic, weak) FLMenuViewScrollView *itemScrollView;
@property (nonatomic, weak) FLMenuViewItem *functionButton;
@property (nonatomic, strong) NSMutableArray *buttons;
@property (nonatomic, strong) FLMenuViewItem *selectedButton;
...
+ (instancetype)pageMenuWithFrame:(CGRect)frame trackerStyle:(FLMenuViewTrackerStyle)trackerStyle {
    FLMenuView *pageMenu = [[FLMenuView alloc] initWithFrame:frame trackerStyle:trackerStyle];
    return pageMenu;
}

- (instancetype)initWithFrame:(CGRect)frame trackerStyle:(FLMenuViewTrackerStyle)trackerStyle {
    if (self = [super init]) {
        self.frame = frame;
...
//    NSAssert(selectedItemIndex < items.count-1, @"selectedItemIndex 大于了 %ld",items.count-1);
    ...
    if (self.buttons.count) {
        ...
        FLMenuViewItem *selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:selectedButton];

        // FLMenuViewTrackerStyleTextZoom和FLMenuViewTrackerStyleNothing样式跟tracker没有关联
...
- (void)removeItemAtIndex:(NSUInteger)itemIndex animated:(BOOL)animated {
    NSAssert(itemIndex <= self.items.count, @"itemIndex超过了items的总个数“%ld”",self.items.count);
    // 被删除的按钮之后的按钮需要修改tag值
    for (FLMenuViewItem *button in self.buttons) {
    ...
    if (itemIndex < self.buttons.count) {
        FLMenuViewItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)removeAllItems {
    ...
    for (int i = 0; i < self.buttons.count; i++) {
        FLMenuViewItem *button = self.buttons[i];
        [button removeFromSuperview];
    }
...
- (nullable NSString *)titleForItemAtIndex:(NSUInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        FLMenuViewItem *item = [self.buttons objectAtIndex:itemIndex];
...
- (void)addButton:(NSInteger)index object:(id)object animated:(BOOL)animated {
    ...
    FLMenuViewItem *button = [FLMenuViewItem buttonWithType:UIButtonTypeCustom];
...
- (BOOL)haveOrNeedsTracker {
    if (self.trackerStyle != FLMenuViewTrackerStyleTextZoom && self.trackerStyle != FLMenuViewTrackerStyleNothing) {
        return YES;
    }
    return NO;
}
...
- (void)setupSubViews {
    // 必须先添加分割线，再添加backgroundView;假如先添加backgroundView,那也就意味着backgroundView是FLMenuView的第一个子控件,而scrollView又是backgroundView的第一个子控件,当外界在由导航控制器管理的控制器中将FLMenuView添加为第一个子控件时，控制器会不断的往下遍历第一个子控件的第一个子控件，直到找到为scrollView为止,一旦发现某子控件的第一个子控件为scrollView,会将scrollView的内容往下偏移64;这时控制器中必须设置self.automaticallyAdjustsScrollViewInsets = NO;为了避免这样做，这里将分割线作为第一个子控件
    FLMenuViewLine *dividingLine = [[FLMenuViewLine alloc] init];
    ...
    FLMenuViewScrollView *itemScrollView = [[FLMenuViewScrollView alloc] init];
    ...
    FLMenuViewItem *functionButton = [FLMenuViewItem buttonWithType:UIButtonTypeCustom];
...
// 按钮点击方法
- (void)buttonInPageMenuClicked:(FLMenuViewItem *)sender {
    ...
    if (self.selectedButton != sender) {
        ...
        if (self.trackerStyle == FLMenuViewTrackerStyleTextZoom || _selectedItemZoomScale != 1) {

            if (labs(toIndex-fromIndex) >= 2) { // 该条件意思是当外界滑动scrollView连续的滑动了超过2页
                for (FLMenuViewItem *button in self.buttons) { // 必须遍历将非选中按钮还原缩放，而不是仅仅只让上一个选中的按钮还原缩放。因为当用户快速滑动外界scrollView时，会频繁的调用-zoomForTitleWithProgress:fromButton:toButton:方法，有可能经过的某一个button还没彻底还原缩放就直接过去了，从而可能会导致该按钮文字会显示不全，所以在这里，将所有非选中的按钮还原缩放
...
- (void)moveItemScrollViewWithSelectedButton:(FLMenuViewItem *)selectedButton {
...
// 移动跟踪器
- (void)moveTrackerWithSelectedButton:(FLMenuViewItem *)selectedButton {
...
// 功能按钮的点击方法
- (void)functionButtonClicked:(FLMenuViewItem *)sender {
...
- (void)prepareMoveTrackerFollowScrollView:(UIScrollView *)scrollView {
    ...
    if (self.trackerFollowingMode == FLMenuViewTrackerFollowingModeAlways) {
        // 这个方法才开始移动跟踪器
    ...
    } else if (self.trackerFollowingMode == FLMenuViewTrackerFollowingModeHalf) {
        FLMenuViewItem *fromButton;
        FLMenuViewItem *toButton;
        if (progress > 0.5) {
    ...
    } else { // self.trackerFollowingMode = FLMenuViewTrackerFollowingModeEnd
        // 什么都不用做
    }
...
- (void)moveTrackerWithProgress:(CGFloat)progress fromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex currentOffsetX:(CGFloat)currentOffsetX beginOffsetX:(CGFloat)beginOffsetX {
    ...
    if (self.trackerStyle == FLMenuViewTrackerStyleLine) {
    ...
    } else if (self.trackerStyle == FLMenuViewTrackerStyleLineAttachment) {
    ...
    } else if (self.trackerStyle == FLMenuViewTrackerStyleTextZoom || self.trackerStyle == FLMenuViewTrackerStyleNothing) {
        // 缩放文字
        if (_selectedItemZoomScale != 1) {
            [self zoomForTitleWithProgress:progress fromButton:fromButton toButton:toButton];
        }
    } else if (self.trackerStyle == FLMenuViewTrackerStyleRoundedRect) {
        newCenter.x = fromButton.center.x + xDistance * progress;
...
- (void)setTrackerStyle:(FLMenuViewTrackerStyle)trackerStyle {
    _trackerStyle = trackerStyle;
    switch (trackerStyle) {
        case FLMenuViewTrackerStyleLine:
        case FLMenuViewTrackerStyleLineLongerThanItem:
        case FLMenuViewTrackerStyleLineAttachment:
            self.tracker.backgroundColor = _selectedItemTitleColor;
            break;
        case FLMenuViewTrackerStyleRoundedRect:
        case FLMenuViewTrackerStyleRect:
            ...
            _trackerHeight = 0;
            break;
        case FLMenuViewTrackerStyleTextZoom:
            // 此样式下默认1.3
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    _selectedItemIndex = selectedItemIndex;
    if (self.buttons.count) {
        FLMenuViewItem *button = [self.buttons objectAtIndex:selectedItemIndex];
        [self buttonInPageMenuClicked:button];
    }
}

- (void)setDelegate:(id<FLMenuViewDelegate>)delegate {
    if (delegate == _delegate) {return;}
    _delegate = delegate;
    if (self.buttons.count) {
        FLMenuViewItem *button = [self.buttons objectAtIndex:_selectedItemIndex];
...
- (void)setPermutationWay:(FLMenuViewPermutationWay)permutationWay {
...
- (void)setCloseTrackerFollowingMode:(BOOL)closeTrackerFollowingMode {
    ...
    if (closeTrackerFollowingMode) {
        self.trackerFollowingMode = FLMenuViewTrackerFollowingModeEnd;
    } else {
        self.trackerFollowingMode = FLMenuViewTrackerFollowingModeAlways;
    }
}
#pragma mark - getter
...
- (void)layoutSubviews {
    ...
    [self.buttons enumerateObjectsUsingBlock:^(FLMenuViewItem *button, NSUInteger idx, BOOL * _Nonnull stop) {
        CGFloat setupButtonW = [[self.setupWidths objectForKey:[NSString stringWithFormat:@"%lu",(unsigned long)idx]] floatValue];
        if (self.permutationWay == FLMenuViewPermutationWayScrollAdaptContent) {
        ...
        } else if (self.permutationWay == FLMenuViewPermutationWayNotScrollEqualWidths) {
            // 求出外界设置的按钮宽度之和
...
- (void)resetSetupTrackerFrameWithSelectedButton:(FLMenuViewItem *)selectedButton {
    ...
    switch (self.trackerStyle) {
        ...
        case FLMenuViewTrackerStyleLineAttachment:
        {
            ...
            trackerH = _trackerHeight;
            trackerX = selectedButton.frame.origin.x;
            trackerY = self.itemScrollView.bounds.size.height - trackerH -7;
            self.tracker.frame = CGRectMake(trackerX, trackerY, trackerW, trackerH);
        }
            break;
        case FLMenuViewTrackerStyleRect:
